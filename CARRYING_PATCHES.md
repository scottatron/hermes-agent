...
| 6 | `feat(terminal): add Kubernetes pod execution backend` | `d19dbf33f` (fork-local, not yet proposed upstream) | Runs agent commands in a Kubernetes pod so the home cluster's resource limits, RBAC, and node placement are the execution boundary, rather than the Hermes host or a third-party sandbox SaaS. The rebase onto `talaria` adapted it to upstream's new `_container_config_from_config()` helper and added the Kubernetes branch to `ensure_task_env()`'s image selection. | Upstream ships an equivalent `terminal.backend: kubernetes` (or a terminal-backend registry that lets this ship as an external package) and the terminal backend/requirements tests pass without this commit. |
